...
function reverse($word1){
    $longPara = strlen($word1);
    $rest = "";
    for ($a = $longPara-1;$a>=0;$a--)
    {
        $rest .= $word1[$a];
    }
    return $rest;
...
function reverseString($pembalikan){
    $dibalik = reverse($pembalikan);
    echo $dibalik . "<br>";
}
...
// Code function di sini
function palidrome($drome){
    $balikpali = reverse($drome);
    if ($balikpali == $drome){
        echo $drome . "=>Benar <br>";
    }
    else{
...
// Hapus komentar di bawah ini untuk jalankan code
 palidrome("civic") ; // true
 palidrome("nababan") ; // true
 palidrome("jambaban"); // false
